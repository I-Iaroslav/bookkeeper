...
class Bookkeeper:
    ...
    def add_expense(self, amount: str,
        ...
        try:
            ...
            raise ValueError('Сумма должна быть целым положительным числом.') from err
        if amount_int <= 0:
            raise ValueError('Сумма должна быть целым положительным числом.')
    ...
    def modify_budget(self, pk: int | None,
        ...
        if new_limit_int < 0:
            self.update_budgets()
            raise ValueError('Не хватает бюджета.')
        # добавление
        if pk is None:
